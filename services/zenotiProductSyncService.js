...
async function syncProducts({ trigger = 'manual' } = {}) {
  ...
  for (const entry of merged.values()) {
    ...
    try {
      ...
      /*
       * Commerce › Products is the pharmacy's own catalogue now (rebuilt from
       * the App Stock template, 2026-09-07) and no longer mirrors Zenoti's
       * product master. Zenoti products land ONLY on the per-centre shelf
       * rows below (Inventory), where the stock side reads them.
       */
      stats.updated += 1;
    } catch (error) {
      stats.errors += 1;
...
async function syncCentreShelves(merged, byCentre) {
  ...
  // Commerce products that happen to share a Zenoti id keep their shelf link;
  // everything else is shelf-only.
