...
exports.getDashboard = async (req, res) => {
  try {
    ...
    const [countRows, dayRows, perfRows, svcRows, paidRows, prevPaid, orders, prevOrders, packages, prevPackages, memberships, prevMemberships] = await Promise.all([
      ...
      ProductOrder.find({ ...appOnly, createdAt: { $gte: start, $lte: end } }).select('pricing paymentStatus paymentMethod orderStatus createdAt userId items fulfilment.type').lean(),
    ...
    const counts = {
      ...
      openOrders: ordersScoped.filter((o) => !['Delivered', 'Collected', 'Cancelled', 'Returned'].includes(o.orderStatus)).length,
      // Store-pickup orders waiting at a desk — the Overview's attention row.
      readyForPickup: ordersScoped.filter((o) => o.orderStatus === 'Ready for Pickup').length,
      openPickupOrders: ordersScoped.filter((o) => o.fulfilment && o.fulfilment.type === 'pickup' && !['Collected', 'Cancelled', 'Returned'].includes(o.orderStatus)).length,
