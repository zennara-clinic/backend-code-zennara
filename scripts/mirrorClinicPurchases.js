...
(async () => {
  ...
  for await (const g of cursor) {
    ...
    for (const [n, c] of o.unmatched || []) unmatched.set(n, (unmatched.get(n) || 0) + c);
    if (tot.guests % 500 === 0) console.log(`  ${tot.guests} guests… ${Math.round((Date.now() - started) / 1000)}s`);
  }
  console.log(JSON.stringify(tot));
  console.log('unmatched sale names (top 40):', [...unmatched.entries()].sort((a, b) => b[1] - a[1]).slice(0, 40).map(([n, c]) => `${n} (${c})`).join(' | '));
